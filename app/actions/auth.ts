...
export async function handleLogin(accessToken: string, email, password) {
  ...
  if (match) {
    ...
    const newRefreshToken = jwt.sign({ userId: foundUser.id }, process.env.REFRESH_TOKEN_SECRET, {
      expiresIn: '1d',
    })

    let newRefreshTokenArray = !accessToken
...
export async function handleRefreshToken(refreshToken) {
  ...
  jwt.verify(refreshToken, process.env.REFRESH_TOKEN_SECRET, async (err, decoded) => {
    ...
    const newRefreshToken = jwt.sign({ username: foundUser.id }, process.env.REFRESH_TOKEN_SECRET, {
      expiresIn: '1d',
    })
    // Saving refreshToken with current user
